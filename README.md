...
Let's now implement the *side effect* that is going to load & dispatch the data and the *reducer* that is going to save the data in the store.
...
## Refactor the edit course feature to use the store

As usual for a new feature, start by defining the action. So, into the CourseActionTypes enum, define CourseSaved action that would be triggered by a click on the save button. So, the action source is [Edit Course Dialog] and the associated event is Course Saved. Now, that we have added this new action into our enum, let’s define the actual class for CourseSaved. It has the type CourseSaved and into construnctor it has a payload that is a little bit different than usual. The payload would contain an object with a property called course which is of type Update. This is a new ngrx specific type that is going to contain all the information needed to update a given entity in memory; the ID of the entity and the properties that have changed. Last bit of defining the action is to add it to the CourseActions union type.

Now in the returned observable after saving the course into the backend successfully, we can dispatch this CourseSaved action to the store with a payload containing the saved course after introducing it as being of type Update, where we need to specify the id of the given course as well as the changes object. A note to be taken, the changes property is of type partial that comes bundled in with the typescript compiler that allows us to define an object with only a few properties of the entity. However, we still have type safety with partial objects that might have less than all the mandatory properties of the entity. So, in this object we could be filling in the changes that we made to the course object by obtaining the properties values from the form’s value object one by one or we could simply assign the whole form’s value object to the changes property. A note to be taken, if the name of a property is the same as the name of variable being assign to it, then you can simplify this assignment by removing the duplicate.

Now that the action has been dispatched, it going to be handled by a case clause in the coursesReducer specific to this type of action that takes the payload and update with it the course in state using the adaptor’s method updateOne. A note to be taken, you can do updateOne if we already know that the course is for sure present inside the store. We can also do upsertOne, this is going to update the course if it already exists and if it does not exist yet it's then going to create a new course. 

So, whenever the course is updated it is going to be broadcasted to any components that need this information. Everything is generic. We simply update the store. The store broadcasts the new version of the state to any subscribed components. The components update themselves transparently without knowing what triggered the change. At this point we can see some of the main benefits of the store architecture. In this case we have a UI with multiple representations of the data, the counter and the courses list. And whenever we edit that data in memory, the UI immediately updates itself transparently without having to contact the server again to recalculate a view model.
